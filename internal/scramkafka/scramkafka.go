...
func NewConsumerClient(clientID string, cfg config.KafkaConfig) (sarama.ConsumerGroup, error) {

	if len(cfg.Group) == 0 {
		return nil, fmt.Errorf("no Kafka consumer group defined, please set the -group flag")
	}

	if len(clientID) == 0 {
		return nil, fmt.Errorf("please set client id format is ${service_name}${uuid}")
	}
